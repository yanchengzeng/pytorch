...
#             if "with torch.cuda._DeviceGuard" in line:
#                 my_rank = int(line.strip(" ").replace("with torch.cuda._DeviceGuard(", "")[0])
#                 break
#         assert my_rank in [0, 1]

#         # Get the world_size
#         my_world_size = None
#         for line in result._lines:
#             if "_find_or_create_pg_by_ranks_and_tag" in line:
#                 my_world_size = int(line[-2])
#                 break
#         assert my_world_size == 2
...
# # kernel path: /tmp/torchinductor_willfeng/ik/cikt6nznvrz4z32luxysgdbyzz5pedsgc65c4hw7anygmrjhimqa.py
...
# @pointwise(size_hints=[16], filename=__file__, meta={'signature': {0: '*fp32', 1: '*fp32', 2: 'i32'}, 'device': 0, 'device_type': 'cuda', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_relu_0', 'configs': [instance_descriptor(divisible_by_16=(0, 1, 2), equal_to_1=(), ids_of_folded_args=(), divisible_by_8=(2,))]})
# @triton.jit
...
#     with torch.cuda._DeviceGuard(${my_rank}):
#         torch.cuda.set_device(${my_rank}) # no-op to ensure context
#         buf0 = empty_strided((4, 4), (4, 1), device='cuda', dtype=torch.float32)
#         buf0.copy_(arg0_1) #no reuse
#         buf1_pg = c10d._find_or_create_pg_by_ranks_and_tag('', ${list(range(my_world_size))}, ${my_world_size})
#         buf1 = buf0
...
#         # Source Nodes: [relu], Original ATen: [aten.relu]
#         stream0 = get_cuda_stream(${my_rank})
#         # triton_poi_fused_relu_0.run(arg0_1, buf3, 16, grid=grid(16), stream=stream0)
#         # buf3.add_(0)  # some dumb in-place op
#         del arg0_1
...
#     arg0_1 = rand_strided((4, 4), (4, 1), device='cuda:${my_rank}', dtype=torch.float32)
...
# """.replace("${my_rank}", str(my_rank)).replace("${my_world_size}", str(my_world_size)).replace("${list(range(my_world_size))}", str(list(range(my_world_size))))

#         result._lines = my_generated_code.splitlines()
